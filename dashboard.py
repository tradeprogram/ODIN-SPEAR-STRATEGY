...
if mode == "SUMMARY":
    ...
    rsi = float(row["RSI"])
    score = float(row.get("점수", row.get("점수(룰)", 0)))
    ret5 = float(row.get("5일수익률", 0))
    p3 = float(row.get("3일확률", row.get("3일상승확률(%)", 50)))
    p5 = float(row.get("5일확률", row.get("5일상승확률(%)", 50)))
    p10 = float(row.get("10일확률", row.get("10일상승확률(%)", 50)))
    signal_raw = row.get("판단", "-")

# MACRO 값 파싱 (없으면 None/빈값)
macro_score = row.get("MACRO_SCORE", None)
try:
    macro_score = float(macro_score) if macro_score not in [None, ""] else None
except Exception:
    macro_score = None
...
# =====================================
# 상단 혼합형 레이아웃 (원래 스타일 유지 + MACRO 추가)
# =====================================
        ...
        st.markdown(
            ...
            ">
                ...
                <div style="font-size: 0.9rem; margin-bottom: 0.2rem;">
                    {macro_signal or "정보 없음"}
                </div>
                <div style="font-size: 0.8rem; opacity: 0.7;">
                    MACRO SCORE: {display_macro_score}
                </div>
            </div>
            """,
            unsafe_allow_html=True,
...
st.subheader("📋 전체 종목 리스트")
st.dataframe(df, use_container_width=True)
>>>>>>> 212c490 (Auto update: 2025-11-29 03:23:53)
